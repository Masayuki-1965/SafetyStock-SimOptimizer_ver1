...
def display_sidebar():
    ...
    with st.sidebar:
        ...
        step_styles = {
            ...
            2: {"main": "STEP 2：安全在庫算出ロジックの体感", "sub": "　　　　（選定機種）"},
            3: {"main": "STEP 3：安全在庫算出と登録データ作成", "sub": "　　　　（全機種）"}
        }
        ...
        with st.expander("Plotlyインタラクティブグラフの使い方", expanded=False):
            ...
            - **データ確認**: グラフ上の線やポイントにマウスを置くと、詳細値がポップアップ表示されます<br>
            - **拡大表示**: グラフ内で見たい期間をドラッグして範囲選択すると拡大表示されます<br>
            - **レンジ(範囲)スライダー**: グラフ下部のスライダーバーで表示期間を調整できます(ハンドルをドラッグして範囲を変更)<br>
            - **表示移動**: 拡大後、右クリックドラッグで表示位置を調整できます<br>
            - **初期表示**: ダブルクリックすると全期間表示に戻ります<br>
