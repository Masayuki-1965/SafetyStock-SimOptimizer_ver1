...
**縦（行）**:
- A区分、B区分、C区分
- 追加区分がある場合も同じ形式で下に追加（例：D区分、E区分 など）
- 最終行は `合計`
...
- 件数の横に括弧で割合%を併記
- 割合%の分母は `商品コードの総件数`（動的に変動する値）
- 計算式：`割合% = 各処理件数 ÷ 商品コードの総件数 × 100`
- 小数点第2位 `xx.xx%` で表示
- 例：`3件（1.51%）` `11件（5.53%）` `138件（69.35%）`

#### 最終行「合計」の表示仕様

- `合計` 行のセル全体に **薄緑背景・緑字** を適用
- 背景色：`#E8F5E9`（薄緑）
- 文字色：`#2E7D32`（緑）
- 件数と割合%は他の区分行と同じ形式で表示（%は括弧併記のみ）
- 例：`24件（12.06%）` のように表示（値は動的）

#### テーブル直下の補足説明

- 必ず1行で追加し、**見た目は既存の補足説明のデザイン・フォント・テイストを維持**
- 実装方法：Streamlitの`st.caption()`を使用
- 文言（固定）：`※ 割合（%）＝ 各処理件数 ÷ 商品コードの総件数 × 100`

#### デザイン変更禁止

- テーブル全体のフォントサイズ、フォント種類、列の配色、凡例、テイストを変更しない
- 修正は **文言の置換・数値フォーマットの追記のみ**
